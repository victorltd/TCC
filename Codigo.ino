...
#define WIFI_PASSWORD "#labccomp#"

//#define WIFI_SSID "DaniloRafael"
//#define WIFI_PASSWORD "Paco1968"
...
#define DATABASE_URL "estacaoaero-default-rtdb.firebaseio.com" //<databaseName>.firebaseio.com or <databaseName>.<region>.firebasedatabase.app
...
void loop()
{
  ...
  int humidade1 = round(humidade);
  int pressao1= round(pressao);
  ...
  if (Firebase.ready() && (millis() - sendDataPrevMillis > 15000 || sendDataPrevMillis == 0))
  {
    ...
    Serial.printf("Set Temperature... %s\n", Firebase.setFloat(fbdo, F("/test/temperature"), temperature1) ? "ok" : fbdo.errorReason().c_str());
    ...
    Serial.printf("Set Humidity... %s\n", Firebase.setDouble(fbdo, F("/test/humidity"), humidade1) ? "ok" : fbdo.errorReason().c_str());

    Serial.printf("Get Humidity... %s\n", Firebase.getDouble(fbdo, F("/test/humidity")) ? String(fbdo.to<double>()).c_str() : fbdo.errorReason().c_str());

     Serial.printf("Set Preassure... %s\n", Firebase.setDouble(fbdo, F("/test/pressao"), pressao1) ? "ok" : fbdo.errorReason().c_str());

    Serial.printf("Get Preassure... %s\n", Firebase.getDouble(fbdo, F("/test/pressao")) ? String(fbdo.to<double>()).c_str() : fbdo.errorReason().c_str());

    Serial.printf("Set AD... %s\n", Firebase.setDouble(fbdo, F("/test/AD"), ad1) ? "ok" : fbdo.errorReason().c_str());

    Serial.printf("Get AD... %s\n", Firebase.getDouble(fbdo, F("/test/AD")) ? String(fbdo.to<double>()).c_str() : fbdo.errorReason().c_str());
...
//      json.add(String(count), humidade);
//      //json.add(String(count), "smart!");
//      Serial.printf("Update node... %s\n", Firebase.updateNode(fbdo, F("/test/json/value/round"), json) ? "ok" : fbdo.errorReason().c_str());

      json.add(String(count), humidade1);
      Serial.printf("Update node... %s\n", Firebase.updateNode(fbdo, F("/test/humidityJ"), json) ? "ok" : fbdo.errorReason().c_str());

      json.add(String(count), temperature1);
      Serial.printf("Update node... %s\n", Firebase.updateNode(fbdo, F("/test/temperatureJ"), json) ? "ok" : fbdo.errorReason().c_str());

      json.add(String(count), pressao1);
      Serial.printf("Update node... %s\n", Firebase.updateNode(fbdo, F("/test/pressaoJ"), json) ? "ok" : fbdo.errorReason().c_str());

      json.add(String(count), ad1);
      Serial.printf("Update node... %s\n", Firebase.updateNode(fbdo, F("/test/ADJ"), json) ? "ok" : fbdo.errorReason().c_str());
